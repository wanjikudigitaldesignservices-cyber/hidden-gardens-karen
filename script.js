document.addEventListener('DOMContentLoaded',()=>{
    ...
    document.getElementById('bookingForm').addEventListener('submit',e=>{
        e.preventDefault();const d=Object.fromEntries(new FormData(e.target));d.id=Date.now();d.submittedAt=new Date().toISOString();d.status='New Inquiry';
        const all=JSON.parse(localStorage.getItem('hiddengardens_inquiries')||'[]');all.push(d);localStorage.setItem('hiddengardens_inquiries',JSON.stringify(all));
        e.target.reset();showToast('Inquiry received. Our events team will contact you shortly.');
    });
});
function bookTour(s){
    document.getElementById('spaceSelect').value=s;
